...
import be.gestatech.bookstore.service.api.EmailService;
...
import be.gestatech.bookstore.service.exception.SystemException;
...
import static be.gestatech.bookstore.service.email.EmailTemplatePart.BODY_TITLE;
import static be.gestatech.bookstore.service.email.EmailTemplatePart.SUBJECT_CONTENT;
import static org.omnifaces.util.Messages.isEmpty;

public abstract class AbstractEmailServiceBean implements EmailService {

    private static final String LOG_MAIL_FAIL = "Failed to send mail [type=%s, to=%s, replyTo=%s, subject=%s]";

    private static final List<String> ALLOWED_FROM_DOMAINS = Arrays.asList("gestatech.be");
    ...
    private Map<String, String> buildTemplateContent(EmailTemplate templateEmail, Map<String, Object> messageParameters) {
        ...
        templateEmail.getTemplateParts().keySet().stream().forEach(key -> templateContent.put(key.getKey(), templateEmail.getTemplateParts().get(key)));
